...
async function deQueue() {
    if (handleEmptyListError()) // Kiểm tra xem danh sách có trống không
        return;

    await deleteNode(0); // Xóa phần tử từ đầu danh sách
    await animateNodesAfterRemove(1); // Di chuyển các phần tử còn lại sang trái
}
async function push(data) {
    // Không cần kiểm tra lỗi cho việc thêm vào đỉnh stack
    ...
    list.insertBefore(pointer, nodes[0]);
    list.insertBefore(node, pointer);
...
async function pop() {
    ...
    await deleteNode(0); // Xóa phần tử từ đỉnh stack
    await animateNodesAfterRemove(1); // Di chuyển các phần tử còn lại lên trên
}